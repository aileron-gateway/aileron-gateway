...
func newEnvProvider(spec *v1.DigestAuthnEnvProvider) (kvs.Commander[string, credential], error) {
...
func newFileProvider(spec *v1.DigestAuthnFileProvider) (kvs.Commander[string, credential], error) {
	store := &kvs.MapKVS[string, credential]{}
	store.Open(context.Background())
