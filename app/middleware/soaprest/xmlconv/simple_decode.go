...
func (s *Simple) parseContent(decoder *xml.Decoder, start xml.StartElement, end xml.EndElement, ns [][2]string) (map[string]any, error) {
	...
	for _, attr := range start.Attr {
		attrs[AttrNameString(attr.Name, s.AttrPrefix, s.NamespaceSep, ns)] = attr.Value
	}
...
// mergeChildren does not check if a key is already exists in the target map
// but overwrite values.
...
//
//	┌───────────────────────────┐┌───────────────────────────┐
//	│==== Input keys-values ====││====== Resulting map ======│
//	└───────────────────────────┘└───────────────────────────┘
//	┌───────────────────────────┐┌───────────────────────────┐
